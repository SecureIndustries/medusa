...
static int tcpsocket_client_onevent (struct medusa_tcpsocket *tcpsocket, unsigned int events, void *context, ...)
{
        ...
        fprintf(stderr, "client   events: 0x%08x / 0x%08x, state: %d\n", events, *cevents, medusa_tcpsocket_get_state(tcpsocket));
        ...
        if (events & MEDUSA_TCPSOCKET_EVENT_WRITE) {
                if (*cevents & MEDUSA_TCPSOCKET_EVENT_WRITE) {
                        fprintf(stderr, "invalid events\n");
                        return -1;
                }
                *cevents |= MEDUSA_TCPSOCKET_EVENT_WRITE;
                if (medusa_tcpsocket_get_state(tcpsocket) != MEDUSA_TCPSOCKET_STATE_CONNECTED) {
                        fprintf(stderr, "invalid state: %d @ %s %s:%d\n", medusa_tcpsocket_get_state(tcpsocket), __FUNCTION__, __FILE__, __LINE__);
                        return -1;
                }
...
static int tcpsocket_server_onevent (struct medusa_tcpsocket *tcpsocket, unsigned int events, void *context, ...)
{
        ...
        if (events & MEDUSA_TCPSOCKET_EVENT_WRITE) {
                if (*levents & MEDUSA_TCPSOCKET_EVENT_WRITE) {
                        fprintf(stderr, "invalid events\n");
                        return -1;
                }
                *levents |= MEDUSA_TCPSOCKET_EVENT_WRITE;
                if (medusa_tcpsocket_get_state(tcpsocket) != MEDUSA_TCPSOCKET_STATE_CONNECTED) {
                        fprintf(stderr, "invalid state: %d @ %s %s:%d\n", medusa_tcpsocket_get_state(tcpsocket), __FUNCTION__, __FILE__, __LINE__);
                        return -1;
                }
...
static int test_poll (unsigned int poll)
{
        ...
        rc = medusa_tcpsocket_set_buffered(tcpsocket, 1);
        if (rc < 0) {
                goto bail;
        }
