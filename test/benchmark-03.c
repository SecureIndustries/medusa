...
static int io_callback (struct medusa_io *io, unsigned int events, void *context)
{
        ...
        if (events & MEDUSA_EVENT_IN) {
                n = recv(medusa_io_get_fd(io), (char *) &ch, sizeof(ch), 0);
                if (n >= 0) {
                        g_count += 1;
                } else {
                        g_failures++;
...
static int test_poll (unsigned int poll)
{
        ...
        for (j = 0; j < g_samples; j++) {
                ...
                for (i = 0; i < g_npipes; i++) {
                        ...
                        rc |= medusa_io_set_fd(io, g_pipes[i * 2]);
                        rc |= medusa_io_set_events(io, MEDUSA_EVENT_IN);
                        rc |= medusa_io_set_callback(io, io_callback, (void *) ((uintptr_t) i));
                ...
                for (i = 0; i < g_nactives; i++, g_fired++) {
                        (void) send(g_pipes[i * space + 1], "e", 1, 0);
                }
